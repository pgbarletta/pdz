#!/bin/bash
#PBS -N temp_larga_each_aa 
#PBS -o temp_larga_each_aa.out
...
do
	...
	echo "trajin /scratch/pbarletta/pdz/run/lb/neq/data/larga_4/T"$i"plb_4.nc" >> $archivo
        for ((j=1;j<133;j++))
        do
            echo temperature T"$j" :"$j" ntc 1 out /scratch/pbarletta/pdz/run/lb/neq/temperatura/each_aa/larga/"$i"larga_temp_each_aa"$j" >> $archivo
        done
	echo "go" >> $archivo
