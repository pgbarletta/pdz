#!/bin/bash
#PBS -N strip_PCA_corta
#PBS -o strip_PCA_corta.out
#PBS -e strip_PCA_corta 
#PBS -j oe
...
do
    ...
    for ((j=1; j<401; j++))
    do
    	echo trajin /scratch/pbarletta/pdz/run/lb/neq/data/corta_3/T"$j"plb_3.nc "$i" "$i" 1 >> $archivo
    done
    ...
    echo trajout /scratch/pbarletta/pdz/run/lb/neq/pca/corta/step_"$i"plb_3.nc >> $archivo
    echo go >> $archivo
