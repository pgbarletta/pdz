...
do
	...
	echo average /scratch/pbarletta/pdz/run/lb/neq/pca/larga/avg_step_"$i"plb_4.pdb  >> $archivo
        echo run >> $archivo
	echo  >> $archivo
	echo # Step two. RMS-Fit to average structure. Calculate covariance matrix. >> $archivo
	echo reference /scratch/pbarletta/pdz/run/lb/neq/pca/larga/avg_step_"$i"plb_4.pdb [avg] >> $archivo
	...
	echo runanalysis diagmatrix mtx_lb vecs 297 out /scratch/pbarletta/pdz/run/lb/neq/pca/larga/"$i"modes_larga >> $archivo
	echo go >> $archivo
