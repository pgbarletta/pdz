...
for paso = 1:180
    in_trj_filename = string(home, neq, "bupca/larga/", "step_", paso, "plb_4.nc")
    in_trj = Trajectory(in_trj_filename)
    
    # Average.
    in_avg_filename = string(home, neq, "bupca/larga/", "avg_step_", paso, "plb_4.pdb")
    ...
    t0_modes, t0_evals = read_ptraj_modes(string(home, neq, "bupca/larga/", paso, "modes_larga"), aa3_cut);
    ...
    writetable(string(home, neq, "bupca/wlarga/", "weights_a_neq_step", paso), df_weights_t0a, separator = '\t')
    writetable(string(home, neq, "bupca/wlarga/", "weights_b_neq_step", paso), df_weights_t0b, separator = '\t')

    writetable(string(home, neq, "bupca/wlarga/", "pnum_neq_step", paso), df_pnum_cnt_a, separator = '\t')
