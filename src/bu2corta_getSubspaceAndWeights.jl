...
for paso = 1:2000
    ...
    #in_avg_filename = string(home, neq, "pca/corta/", "avg_step_", paso, "plb_3.pdb")
    ...
    t0_modes, t0_evals = read_ptraj_modes(string(home, neq, "pca/corta/", paso, "modes_corta"), aa3_cut);
    ...
    writetable(string(home, neq, "bu2pca/wcorta/", "weights_a_neq_step", paso), df_weights_t0a, separator = '\t')
    writetable(string(home, neq, "bu2pca/wcorta/", "weights_b_neq_step", paso), df_weights_t0b, separator = '\t')

    writetable(string(home, neq, "bu2pca/wcorta/", "pnum_neq_step", paso), df_pnum_cnt_a, separator = '\t')
